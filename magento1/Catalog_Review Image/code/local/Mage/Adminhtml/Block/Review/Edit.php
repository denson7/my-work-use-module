...
class Mage_Adminhtml_Block_Review_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        if( $this->getRequest()->getParam($this->_objectId) ) {
             ...
             * 如果评论有图片，获取图片
             */
            $images = Mage::getModel('xreview/image')->getCollection()
                ->addfieldtofilter('review_id',$this->getRequest()->getParam($this->_objectId));
            if(count($images->getItems(0))){
                Mage::register('review_image',$images);
            }else{
                Mage::register('review_image',null);
            }
