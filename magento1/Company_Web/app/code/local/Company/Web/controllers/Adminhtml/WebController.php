...
class Company_Web_Adminhtml_WebController extends Mage_Adminhtml_Controller_action
{
	...
	public function deleteAction() {
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				$model = Mage::getModel('web/web');
				if($this->deleteImage($this->getRequest()->getParam('id'))){
	 ...
	 * @return bool
	 */
	public function deleteImage($webid)
	{
		$data = Mage::getModel('web/web')->load($webid);
		$filename = $data->getFilename();
		$mediaurl=Mage::getBaseDir('media');
		$pathurl = $mediaurl.DS.$filename;
		if(unlink($pathurl)){
			return true;
		}else{
			return false;
